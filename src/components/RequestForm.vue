<template>
  <form class="ll-card" novalidate @submit.prevent="handleSubmit">
    <div class="ll-card__header">
      <h5 class="m-0 text-wine">
        <i class="bi bi-file-earmark-medical me-2"></i
        >{{ isEditing ? 'Edit Emergency Request' : 'New Emergency Request' }}
      </h5>
    </div>
    <div class="ll-card__body">
      ...
      <div class="ll-form-group position-relative">
        ...
        <div class="position-relative">
          ...
          />
          <ul
            v-if="showHospitalSuggestions && filteredHospitals.length"
            class="ll-autocomplete-dropdown"
          >
            <li v-for="h in filteredHospitals" :key="h.name" @mousedown="selectHospital(h)">
              <i class="bi bi-hospital text-wine me-2"></i> {{ h.name }}
              <span class="text-muted small">({{ h.city }})</span>
            </li>
          </ul>
        </div>
        <div v-if="errors.hospitalName" class="invalid-feedback d-block">
          {{ errors.hospitalName }}
        </div>
      </div>
        ...
        <div class="col-md-6 ll-form-group position-relative">
          ...
          <div class="position-relative">
            ...
            <ul
              ...
              class="ll-autocomplete-dropdown"
            >
              <li v-for="c in filteredCities" :key="c" @mousedown="selectCity(c)">
      ...
      <div class="row g-3">
        <div class="col-md-6 ll-form-group">
          ...
          <select
            ...
            class="form-select"
            :class="{ 'is-invalid': errors.urgency }"
        ...
        <div class="col-md-6 ll-form-group">
          ...
          <input
            id="request-units"
            v-model.number="form.unitsNeeded"
            class="form-control"
            :class="{ 'is-invalid': errors.unitsNeeded }"
            type="number"
            min="1"
            max="20"
            ...
            autocomplete="off"
          />
          <div v-if="errors.unitsNeeded" class="invalid-feedback d-block">
            {{ errors.unitsNeeded }}
          </div>
        </div>
      </div>
      ...
      <div class="ll-form-group">
        ...
        <textarea
          ...
          rows="4"
          placeholder="Brief details about emergency patient..."
      ...
      <div class="ll-form-group">
        ...
        <input
          ...
          aria-label="Representative Phone Number"
          autocomplete="tel"
        />
        <div v-if="errors.contactInfo" class="invalid-feedback d-block">
          {{ errors.contactInfo }}
        </div>
      </div>
      ...
      <div class="ll-form-group">
        <label for="request-publish-date"
          >Publish Date &amp; Time (Leave empty to use current time)</label
        >
        <input
...
function validate() {
  ...
  if (!form.hospitalName || form.hospitalName.length < 3)
  ...
  if (!Number.isInteger(form.unitsNeeded) || form.unitsNeeded < 1 || form.unitsNeeded > 20)
    errors.unitsNeeded = 'Units needed must be an integer from 1 to 20.'
