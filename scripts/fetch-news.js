    ...
    res.on('end', () => {
      try {
        ...
        while ((match = itemRegex.exec(data)) !== null) {
          ...
          if (titleMatch) {
            ...
            if (lowerTitle.includes('hiến máu') || lowerTitle.includes('truyền máu')) {
              ...
              lowerTitle.includes('quy định') ||
              lowerTitle.includes('chính sách') ||
              lowerTitle.includes('bộ y tế')
            ) {
              category = 'Policy'
            } else if (
              lowerTitle.includes('bệnh') ||
              lowerTitle.includes('khẩn cấp') ||
              lowerTitle.includes('dịch')
            ) {
              category = 'Emergency'
            } else if (
              lowerTitle.includes('nghiên cứu') ||
              lowerTitle.includes('khoa học') ||
        ...
        if (items.length > 0) {
          ...
          console.log(
            `Successfully fetched and pre-seeded ${items.length} real articles from VnExpress.`
          )
        } else {
